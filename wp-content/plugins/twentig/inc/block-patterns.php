...
function twentig_register_block_pattern( $pattern_name, $pattern_properties ) {
	...
	if ( 'twentytwentytwo' === get_template() ) {
		$pattern_properties['content'] = twentig_replace_colors_for_pattern( $pattern_properties['content'] );
	}

	if ( ! in_array( get_template(), ['twentytwentythree','twentytwentytwo','twentytwentyone', 'twentytwenty' ] ) ) {
...
function twentig_replace_colors_for_pattern( $content ) {
	...
	$content = str_replace(
		...
		[
			...
			'"iconColorValue":"var(--wp--preset--color--foreground)"'
		],
		$content
	);
