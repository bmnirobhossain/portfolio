...
function pix_responsive_css_class($valueString, $id = false){
    if(empty($id) || $id=='') $id = 'custom-responsive-'.rand(1,200000000);
    $handle = 'pix-'. $id;
...
function pixfort_scripts(){
    if( pix_plugin_get_option('pix-custom-cursor') ){
        wp_enqueue_script( 'pix-main-pixfort', PIX_CORE_PLUGIN_URI . 'dist/front/index.bundle.js', array('jquery', 'jquery-ui-core', 'pix-bootstrap-js', 'pix-flickity-js'), PIXFORT_PLUGIN_VERSION, true );
        $jsOptions = array(
            'ENABLE_CURSOR' => true,
            'DISABLE_DEFAULT_CURSOR' => false
        );
