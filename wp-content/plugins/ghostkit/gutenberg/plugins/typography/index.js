    ...
    if (isExist(typographyData.ghostkit_typography)) {
      Object.keys(typographyData.ghostkit_typography).forEach((typography) => {
        if (isExist(typography)) {
          const { fontFamily } = typographyData.ghostkit_typography[typography];
          const { fontFamilyCategory } = typographyData.ghostkit_typography[typography];
          let { fontWeight } = typographyData.ghostkit_typography[typography];
          const fontWeights = [];
          ...
          if (isExist(typographyPrepeareStyles[key].output)) {
            ...
            Object.keys(conformityAttributes).forEach((propertyKey) => {
              if (
                ...
                typographyStyles = `${typographyStyles + propertyKey}: ${
                  typographyPrepeareStyles[key]['style-properties'][propertyKey]
                };`;
              }
            });
...
class TypographyModal extends Component {
  ...
  getPlaceholders(key, isGlobal) {
    ...
    Object.keys(placeholders).forEach((placeholderKey) => {
      const defaultProperty = isExist(customTypographyList[key].defaults[placeholderKey])
      ...
      if (isGlobal) {
        ...
        placeholders[placeholderKey] = isExist(
          this.state.globalCustomTypography[key][conformityAttributes[placeholderKey]]
        )
          ? this.state.globalCustomTypography[key][conformityAttributes[placeholderKey]]
          : defaultProperty;
      }
    ...
    if (customTypography && this.state.globalCustomTypography === false) {
      this.setState({
        ...
        globalAdvanced: getInitialAdvancedState(
          getCustomTypographyList(customTypography.ghostkit_typography, false)
        ),
      });
    }
  }
  ...
  render() {
    const { updateMeta, updateCustomTypography, onRequestClose } = this.props;
  ...
  render() {
    ...
    return (
      <Fragment>
        ...
        {isModalOpen ? (
          <TypographyModalWithSelect onRequestClose={() => this.setState({ isModalOpen: false })} />
        ) : (
          ''
        )}
      </Fragment>
