...
class Api {
	...
	public function library_data() {
		...
		foreach ( $data as $key => $value ) {
			...
			if ( 'theme-data' === $key ) {
				$content = $this->inject_theme_like( $content );
			}
	 ...
	 * @return array
	 */
	public function inject_theme_like( $data ) {
		$liked = Meta_Option::instance()->get_option( 'liked_layout' );

		foreach ( $data as $key => $item ) {
			$data[ $key ]->like = in_array( $item->id, $liked, true );
		}
	...
	public function inject_section_like( $data ) {
		...
		foreach ( $data as $key => $item ) {
			$data[ $key ]->like = in_array( (int) $item->id, $liked, true );
		}
	 ...
	 * Like State.
	 *
	 * @param int    $id ID of single layout / theme.
	 * @param string $state like state.
	 * @param string $option Name of option.
	 *
	 * @return boolean
	 */
	public function like_state( $id, $state, $option ) {
		...
		if ( $state ) {
			...
			$search = array_search( $id, $liked, true );
			if ( false !== $search ) {
				unset( $liked[ $search ] );
			}
		}
	...
	public function section_like_state( $request ) {
		$id    = $this->esc_data( $request->get_param( 'id' ), 'integer' );
		$state = $this->esc_data( $request->get_param( 'state' ), 'boolean' );

		return $this->like_state( $id, $state, 'liked_section' );
	}
	...
	public function layout_like_state( $request ) {
		...
		return $this->like_state( $id, $state, 'liked_layout' );
	}
	...
	public function update_library_data() {
		...
		$apipath   = GUTENVERSE_LIBRARY_URL . 'wp-json/gutenverse-server/v2/';
