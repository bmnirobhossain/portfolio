<?php
function pixfort_char_code_at( $str, $index ) {
$utf16 = mb_convert_encoding( $str, 'UTF-16LE', 'UTF-8' );
return ord( $utf16[ $index * 2 ] ) + ( ord( $utf16[ $index * 2 + 1 ] ) << 8 );
}

function pixfort_get_domain() {
$domain = trim( preg_replace( '/^www\./','', $_SERVER['HTTP_HOST'] ) );
...
function pixfort_xpf( $str ) {
$res = 5381;
for ( $i = strlen( $str ); $i > 0; $i ) {
$res = ( 33 * $res ^ pixfort_char_code_at( $str, --$i ) ) & 0xffffffff;
}
return $res;
}

function pixfort_key( $envato_key ) {
$nk = pixfort_xpf( pixfort_get_domain() );
return 'ftxw8' . ( $nk * 30 ) . '-' . $nk;
}

$essentials_envato_key = '1L0veY0uP1xF0rt';
update_option( 'envato_purchase_code_27889640', $essentials_envato_key );
update_option( 'pixfort_key', pixfort_key( $essentials_envato_key ) );
...
( strpos( $demos[ $i ]['import_file_url'], '/demo-content/gallery/' ) !== false )
) {
$offset = $i;
break;
}
...
define('ESSENTIALS_THEME_VERSION', '3.0.5');
...
function essentials_scripts() {
	...
	if (pix_get_option('pix-automatic-popup')) {
		if (pix_show_automatic_popup()) {
			...
			$main_values['dataAutoPopupOptionId'] = pix_get_option('pix-automatic-popup-id');
...
function pix_theme_admin_scripts() {
	...
	if ( ! wp_script_is( 'jquery-confirm-js', 'enqueued' ) && ! wp_script_is( 'jquery-confirm', 'enqueued' ) ) {
...
add_action('init', function () {
	if (function_exists('pll_register_string')) {
		...
		if (pix_get_option('banner-btn-link')) {
			pll_register_string('essentials-banner-btn-link', pix_get_option('banner-btn-link'));
		}
		...
		if (pix_get_option('banner-btn-link')) {
			icl_register_string('Theme', 'essentials-banner-btn-link', pix_get_option('banner-btn-link'));
		}
		if (pix_get_option('pix-cookies-text')) {
