    ...
    function pix_get_header_button($opts){
       ...
       if(!empty($btn_popup_id)){
        ...
        <div class="d-inline-flex align-items-center d-inline-block2 text-sm mb-0">
            <a data-anim-type="<?php echo esc_attr($animation); ?>" class="btn <?php echo esc_attr( $classes ); ?> d-flex2 align-items-center mr-0" href="<?php echo esc_url( $url ); ?>" <?php echo esc_attr( $target_out) . ' style="' . esc_attr( $custom ); ?>" data-popup-link="<?php echo esc_attr($popup_data);?>" >
            <?php 
                if (  !empty( $btn_icon ) && empty($btn_icon_position) ) { ?>
                    <i class="<?php echo esc_attr($btn_icon); ?> mr-2"></i>
            <?php } ?>
                <span><?php echo do_shortcode($text); ?></span>
            <?php 
                if (  !empty( $btn_icon ) && !empty($btn_icon_position) ) { ?>
                    <i class="<?php echo esc_attr($btn_icon); ?> ml-2"></i>
            <?php } ?>
            </a>
        </div>
        <?php
...
if( !function_exists('pix_get_header_cart') ){
    function pix_get_header_cart($opts){
        ...
        ?>
        <a data-anim-type="<?php echo esc_attr($animation); ?>" href="<?php echo esc_url( home_url( '/' ) ); ?>cart" data-e-disable-page-transition="true" class="btn pix-header-btn btn-link m-0 p-0 pix-header-text pix-px-15 fly-sm2 pix-cart-btn pix-open-sidebar text-<?php echo esc_attr( $color ); ?> d-inline-flex align-items-center <?php echo esc_attr($animation_class); ?>"><i class="pixicon-bag-2 text-18 scale2 position-relative font-weight-bold text-<?php echo esc_attr( $color ); ?>" <?php echo esc_attr( $custom ); ?> ></i><span class="cart-count woo-cart-count badge-pill bg-primary"><?php echo esc_attr( $i_count ); ?></span></a>
        <?php
    }
}
    ...
    extract(shortcode_atts(array(
        ...
        if(in_array($area, $desktop_areas)){
            if($menu_style=='hidden'){
                ...
                $menuBtn = '<a class="hamburger--spin hamburger normal-menu-toggle d-flex '. esc_attr($hidden_state) .'" href="#" data-target="#navbarNav-'. esc_attr( $nav_id ).'">
                    <span class="hamburger-box">
            ...
            if($themeDefault){
                ...
                if( !empty($area) && ($area == 'm_header' || $area == 'm_stack' || $area == 'm_topbar') && pix_get_option('mobile-logo-img') && pix_get_option('mobile-logo-img')['url']  ){
                    ...
                    <a class="navbar-brand" href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr($target); ?>" rel="home">
                        <img src="<?php echo esc_url( $custom_logo_url ); ?>" alt="<?php echo esc_attr($altText); ?>" height="<?php echo esc_attr($heightVal); ?>" width="<?php echo esc_attr($widthVal); ?>" style="<?php echo esc_attr( $img_style ); ?>">
                    </a>
                    <?php
                }else{
